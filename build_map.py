...
build_map.py
────────────
Lit map.config.yaml et génère :
  • www/style.json         — style MapLibre complet
  • granulometry.json      — règles LOD pour apply_granulometry.py
  • pmtiles_params.json    — zoom par couche, lu par generate_pmtiles.bash

Usage :
  python3 build_map.py                          # config par défaut
  python3 build_map.py --config other.yaml      # autre carte
  python3 build_map.py --only style             # juste le style
  python3 build_map.py --only granulometry
  python3 build_map.py --only pmtiles
"""
...
def color(c):
    ...
    s = str(c).strip().lower()
    return COLORS.get(s, str(c))

def lc(layers, name):
    """Config d'une couche avec valeurs par défaut."""
    r = (layers.get(name) or {})
    ap = r.get("appear_at", 10)
    return {
        "label":        r.get("label", name.capitalize()),
        "color":        color(r.get("color")),
        "border_color": color(r.get("border_color")),
...
def sc(cfg, key):
    """Config d'un sous-type avec fallback couche."""
    s = (cfg["subtypes"].get(key) or {})
    return {
        "color":     color(s.get("color", cfg["color"])),
        "appear_at": s.get("appear_at", cfg["appear_at"]),
        "opacity":   s.get("opacity",   cfg["opacity"]),
    }

def zoom(pairs):
    """Expression interpolate/zoom MapLibre. 1 pair → scalaire."""
...
# ── Générateurs de layers MapLibre ────────────────────────────────────────────

def bg(c): return {"id": "background", "type": "background", "paint": {"background-color": c}}
...
def water(cfg):
    ...
    ws = sc(cfg, "wetland")
    wc = color((st.get("wetland") or {}).get("color", "#d4e2c6"))
    wo = (st.get("wetland") or {}).get("opacity", 0.5)
    ...
    for ww, (w, default_z) in {
        "river":  ([(10,1),(18,12)], 10), "canal":  ([(10,1),(18,10)], 10),
        "stream": ([(13,.5),(18,3)], 13), "ditch":  ([(14,.3),(18,2)], 14),
    }.items():
        az = (st.get(ww) or {}).get("appear_at", default_z)
        out.append({"id": f"waterway-{ww}", "type": "line",
            "source": "water", "source-layer": "water", "minzoom": az,
            "filter": ["all", ["==", ["get", "waterway"], ww],
                              ["!=", ["get", "tunnel"], "yes"],
                              ["==", ["geometry-type"], "LineString"]],
            "paint": {"line-color": c, "line-width": zoom(w)}})
    return out

def green(cfg):
    st = cfg["subtypes"]
    FILTERS = {
        "forest":    ["any", ["==", ["get", "landuse"], "forest"],
                             ["==", ["get", "natural"], "wood"]],
        "park":      ["==", ["get", "leisure"], "park"],
        "garden":    ["==", ["get", "leisure"], "garden"],
        "grass":     ["any", ["==", ["get", "landuse"], "grass"],
                             ["==", ["get", "landuse"], "meadow"]],
    ...
    for key, filt in FILTERS.items():
        ...
        c = s["color"]
        if not c: continue
        l = {"id": f"green-{key}", "type": "fill",
             "source": "green", "source-layer": "green",
             "filter": filt, "paint": {"fill-color": c}}
        if s["appear_at"] > 10: l["minzoom"] = s["appear_at"]
        if abs(s["opacity"] - 1.0) > 0.01: l["paint"]["fill-opacity"] = s["opacity"]
        out.append(l)
    return out
...
def trees(cfg):
    ...
    return [
        {"id": "trees-hedge", "type": "line",
         ...
         "paint": {"line-color": "#6ba048",
                   "line-width": zoom([(ha, .8), (18, 3.5)])}},
        {"id": "trees-row", "type": "symbol",
         ...
         "paint": {"text-color": c, "text-halo-color": "#f2efe9",
                   "text-halo-width": .8}},
        {"id": "trees-tree", "type": "symbol",
         "source": "trees", "source-layer": "trees", "minzoom": ta,
         "filter": ["all", ["==", ["geometry-type"], "Point"],
                           ["==", ["get", "natural"], "tree"]],
         "layout": {"text-field": ["match", ["get", "leaf_type"],
                                   "needleleaved", "▲", "●"],
...
def leisure(cfg):
    ...
    for k in st:
        s = sc(cfg, k)
        if s["color"]: fc[k] = s["color"]

    fill_expr = ["match", ["get", "leisure"]]
    for k, c in fc.items(): fill_expr += [k, c]
...
def roads(cfg):
    ...
    for group, (hw_vals, casing_c, cw, fw) in CLASSES.items():
        s    = (st.get(group) or {})
        ap   = s.get("appear_at", base)
        fc   = color(s.get("color", cfg["color"] or "#ffffff"))
        hw_f = ["any"] + [["==", ["get", "highway"], v] for v in hw_vals]

        for variant, extra_f, op in [
            ("surface", ["!=", ["get", "tunnel"], "yes"], 1.0),
            ("tunnel",  ["==", ["get", "tunnel"], "yes"], 0.5),
        ]:
            f = ["all", hw_f, extra_f]
            layout = {"line-cap": "round", "line-join": "round",
                      "line-sort-key": ["coalesce", ["to-number", ["get", "layer"]], 0]}
            out.append({"id": f"roads-casing-{group}-{variant}", "type": "line",
                "source": "roads", "source-layer": "roads",
                "minzoom": ap, "filter": f, "layout": layout,
                "paint": {"line-color": casing_c, "line-width": zoom(cw),
                          "line-opacity": op,
                          **({"line-dasharray": [2,2]} if variant == "tunnel" else {})}})
            out.append({"id": f"roads-fill-{group}-{variant}", "type": "line",
                "source": "roads", "source-layer": "roads",
                "minzoom": ap, "filter": f, "layout": layout,
                "paint": {"line-color": fc, "line-width": zoom(fw),
                          "line-opacity": op}})

    out.append({"id": "road-labels", "type": "symbol",
        "source": "roads", "source-layer": "roads",
        "minzoom": cfg["labels_at"],
        "filter": ["any", ["==", ["get", "highway"], "primary"],
...
def pedestrian(cfg):
    ...
    for key, (hw, dz, wp, dashed) in DEFS.items():
        ...
        p  = {"line-color": c, "line-width": zoom(wp)}
        if dashed: p["line-dasharray"] = [2, 2]
        out.append({"id": f"pedestrian-{key}", "type": "line",
            "source": "pedestrian", "source-layer": "pedestrian",
            "minzoom": ap, "filter": ["==", ["get", "highway"], hw], "paint": p})
    return out

def cycleway(cfg):
    return [{"id": "cycleway", "type": "line",
        "source": "cycleway", "source-layer": "cycleway",
        "minzoom": cfg["appear_at"],
    ...
    for rw, (ties_w, core_w, dz) in DEFS.items():
        ...
        if ties_w:
            out.append({"id": f"railway-{rw}-ties", "type": "line",
                "source": "railway", "source-layer": "railway",
                "minzoom": ap, "filter": filt,
                "layout": {"line-join": "round"},
                "paint": {"line-color": c, "line-width": zoom(ties_w),
                          "line-dasharray": [.2, 4]}})
        out.append({"id": f"railway-{rw}", "type": "line",
            "source": "railway", "source-layer": "railway",
            "minzoom": ap, "filter": filt,
            "layout": {"line-join": "round"},
            "paint": {"line-color": c, "line-width": zoom(core_w)}})
    return out

def public_transport(cfg):
    c  = cfg["color"] or "#e3004f"
    ap = cfg["appear_at"]
    la = cfg["labels_at"]
    return [
        {"id": "public_transport-casing", "type": "line",
         "source": "public_transport", "source-layer": "public_transport",
         "minzoom": ap, "layout": {"line-join": "round", "line-cap": "round"},
         "paint": {"line-color": "#ffffff",
                   "line-width": zoom([(ap,3),(18,9)]), "line-opacity": .6}},
        {"id": "public_transport-line", "type": "line",
         "source": "public_transport", "source-layer": "public_transport",
         "minzoom": ap, "layout": {"line-join": "round", "line-cap": "round"},
         "paint": {"line-color": ["coalesce", ["get", "colour"], c],
...
def boundaries(cfg):
    return [{"id": "boundaries", "type": "line",
        "source": "boundaries", "source-layer": "boundaries",
        "minzoom": cfg["appear_at"],
...
def build_style(config):
    L  = config.get("layers", {})
    M  = config.get("map", {})
    bgc = color(M.get("background", "#f2efe9"))
    gl  = M.get("glyphs",
...
LAYER_TAG = {
    "landuse":"landuse","green":"landuse","water":"waterway","trees":"natural",
    "buildings":"building","leisure":"leisure","roads":"highway",
    "pedestrian":"highway","railway":"railway","poi":"amenity","boundaries":"boundary",
}
    ...
    for name, raw in L.items():
        ...
        if name == "roads":
            ...
            for pt, scfg in st.items():
                pap = (scfg or {}).get("appear_at", ap)
                if pap > 10:
                    rules.append({"match": {"amenity": [pt]},
...
def main():
    p = argparse.ArgumentParser(description="map.config.yaml → style + granulometry + pmtiles")
    ...
    print(f"→ {cfg_path}  ({n_layers} couches)")
    ...
    if only in (None, "granulometry"):
        ...
        total_rules = sum(len(v["rules"]) for v in g["layers"].values())
        print(f"✓  {args.gran_out}  ({total_rules} règles LOD)")
